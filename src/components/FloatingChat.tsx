...
const getSuggestions = (meds: Medicine[]): string[] => {
  const med = meds[0];
  const name = med?.mark || med?.dci || med?.name;
  if (!name) return [];
  return [
...
const FloatingChat = ({ isOpen, onClose, medicines = [] }: FloatingChatProps) => {
                      ...
                      {medicines.length > 0 ? (
                        <div className="space-y-3">
                          <p className="text-sm text-muted-foreground">
                            Ask about the medicines found
                          </p>
                          <div className="flex flex-col gap-1.5">
                            {getSuggestions(medicines).map((suggestion) => (
                              <button
                                key={suggestion}
                                onClick={() => handleSearch(suggestion)}
                                className="text-xs text-left px-3 py-2 rounded-xl bg-muted hover:bg-primary/10 hover:text-primary transition-colors"
                              >
                                {suggestion}
                              </button>
                            ))}
